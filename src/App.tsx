...
import { 
  FileText, BrainCircuit, Users, Gavel, Loader2, MessageCircle,
  ChevronRight, Mic, Search, Scale, Landmark, Scroll,
  Bookmark, History, Menu, FileSearch
} from 'lucide-react';
...
function App() {
  ...
  const renderPage = () => {
    ...
    switch (currentPage) {
      ...
      case 'find': 
        return <AdvocateFinder onProfileSelect={(l) => { 
          setSelectedLawyer(l); 
          setCurrentPage('lawyer-profile'); 
        }} />;
      case 'lawyer-profile': 
        return <AdvocateProfile lawyer={selectedLawyer} onBack={() => setCurrentPage('find')} />;
      case 'ipc': 
        return <IPCLookup />;
      case 'penalty': 
        ...
        <div className="bg-white rounded-2xl shadow-xl border-t-4 border-amber-500 overflow-hidden p-2">
          {items.map((item) => (
            <button 
              ...
              className="w-full text-left px-4 py-3 text-sm text-slate-600 hover:bg-slate-50 hover:text-slate-900 rounded-xl flex items-center gap-3 transition-all group/item"
            >
              <div className="p-2 bg-slate-50 rounded-lg text-slate-400 group-hover/item:bg-slate-900 group-hover/item:text-white transition-all shadow-sm">
        ...
        <header className="bg-white/90 backdrop-blur-xl border-b border-slate-200 sticky top-0 z-50 transition-all duration-300">
          <nav className="container mx-auto px-6 h-20 flex justify-between items-center">
            ...
            <div className="flex items-center gap-3 cursor-pointer group" onClick={() => setCurrentPage('home')}>
              ...
              <h1 className="text-2xl font-serif font-bold text-slate-900 tracking-tight group-hover:text-slate-700 transition-colors">
                Nyay Saathi
              </h1>
            </div>
            ...
            <div className="hidden lg:flex items-center gap-1">
              <button 
                ...
                className={`px-5 py-2 rounded-full font-bold text-sm uppercase tracking-wide transition-all ${
                  currentPage === 'home' ? 'bg-slate-900 text-white shadow-md' : 'text-slate-600 hover:bg-slate-100'
                }`}
              >
                Home
              </button>
              ...
              <button 
                ...
                className="ml-4 px-6 py-2.5 bg-slate-900 text-white rounded-full font-bold hover:bg-amber-500 hover:text-white transition-all shadow-lg hover:shadow-xl text-sm"
              >
                Logout
              </button>
            </div>
